// Lisa de categorias globais
import prisma from "../config/prisma";
import { TransactionType } from "../schemas/transaction.schema";

interface GlobalCategoryInput {
  name: string;
  color: string;
  type: "expense" | "income";
}
  ...
  for (const category of globalCategories) {
    try {
      ...
      if (!existing) {
        ...
        console.log(`✅ Categoria global criada: ${newCategory.name}`);
        createdCategories.push(newCategory);
      } else {
        console.log(`⚠️ Categoria global já existe: ${existing.name}`);
        createdCategories.push(existing);
      }
    } catch (err) {
      console.error(`❌ Erro ao criar categoria global ${category.name}:`, err);
    }
  }
